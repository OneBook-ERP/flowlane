...
// these exact mutators, never a per-view copy. Simulating a "Diagram mount"
// edit and reading it back the way a "Wizard/Table mount" would (findStep /
// state.steps) proves Table, Wizard and Diagram can never diverge — they are
// three views over the one reactive store object. `call` (the network layer)
// is mocked so this stays a pure store test with no live save.
...
describe('createMapStore — single source of truth across mount points', () => {
  it('a field edit made through one mount is visible to every other reader', () => {
    ...
    // StepInspector mount, or DiagramNodePanel's StepInspector mount.
